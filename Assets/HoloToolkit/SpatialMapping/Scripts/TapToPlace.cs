        ...
        private void Update()
        {
            ...
            if (placing)
            {
                    ...
                    30.0f, spatialMappingManager.LayerMask))
                {
                    ...
                    // 6-WORLD-COORDS - position the model away a little from the spatial
                    // mesh and smooth it's motion
                    var pivot = headPosition + gazeDirection * (hitInfo.distance - 0.2f);
                    transform.position = Vector3.Lerp(transform.position, pivot, 0.2f);
